...
def encode_texts(task, texts, batch_size=128):
    return np.vstack([task.TextImage.predict(
...
def experiment(net, data, run_config):
    ...
    with open("result.json", "w") as out:
        for epoch in range(last_epoch+1, run_config['epochs'] + 1):
            costs = dict(SpeechText=Counter(), SpeechImage=Counter(),
                         TextImage=Counter(), SpeechTranscriber=Counter())
            ...
            for _j, items in enumerate(zip(data['SpeechImage'].iter_train_batches(reshuffle=True),
                ...
                j = _j + 1
                item = dict(SpeechImage=items[0], SpeechText=items[1],
                            TextImage=items[2], SpeechTranscriber=items[3])
                for name, task in run_config['tasks']:
                    ...
                    print(epoch, j, j*data[name].batch_size, name, spk, "train",
